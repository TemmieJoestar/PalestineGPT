...
Matrix* create_matrix(int rows, int cols, bool requires_grad) {
    ...
    if (m == NULL) {
        FATAL_ERROR("Failed to allocate memory for Matrix");
    }
    m->rows = rows;
    m->cols = cols; 
    ...
    m->data = calloc(total_elements, sizeof(float));
    if (m->data == NULL) {
        FATAL_ERROR("Failed to allocate memory for DATA");
    }

    if (requires_grad){
        m->grad = calloc(total_elements, sizeof(float));
        if (m->grad == NULL) {
...
void matrix_sigmoid(Matrix* Input, Matrix* Output){
    if (Input->rows != Output->rows || Input->cols != Output->cols) {
        FATAL_ERROR("Dimension mismatch in matrix_sigmoid.");
    }
...
void matrix_softmax(Matrix* Input, Matrix* Output) { 
    if (Input->rows != Output->rows || Input->cols != Output->cols) {
        FATAL_ERROR("Dimension mismatch in matrix_softmax.");
    }
    
    for (int i = 0; i < Input->rows; i++) {
        int row_offset = i * Input->cols;
        
        float row_max = Input->data[row_offset];
        for (int j = 1; j < Input->cols; j++) {
            float val = Input->data[row_offset + j];
            if (val > row_max) row_max = val;
        }
        
        float row_sum = 0.0f;
        for (int j = 0; j < Input->cols; j++) {
            float e = expf(Input->data[row_offset + j] - row_max);
            Output->data[row_offset + j] = e;
            row_sum += e;
        }
        
        float inv_sum = 1.0f / row_sum;
        for (int j = 0; j < Input->cols; j++) {
            Output->data[row_offset + j] *= inv_sum;
        }
    }
}
...
void matrix_addition(Matrix* a, Matrix* b, Matrix* Output) {
    if (a->rows != b->rows || a->cols != b->cols || 
        a->rows != Output->rows || a->cols != Output->cols) {
        FATAL_ERROR("Matrices must be the same size for addition.");
    }
...
void matrix_subtraction(Matrix* A, Matrix* B, Matrix* Output) {
    if (A->rows != B->rows || A->cols != B->cols || 
        A->rows != Output->rows || A->cols != Output->cols) {
        FATAL_ERROR("Matrices must be the same size for subtraction.");
    }
...
void matrix_scalar_multiply(Matrix* Input, float scalar, Matrix* Output) {  
    if (Input->rows != Output->rows || Input->cols != Output->cols) {
        FATAL_ERROR("Dimension mismatch in matrix_scalar_multiply.");
    }
...
void matrix_scalar_subtraction(Matrix* Input, float scalar, Matrix* Output){
    if (Input->rows != Output->rows || Input->cols != Output->cols) {
        FATAL_ERROR("Dimension mismatch in matrix_scalar_subtraction.");
    }
...
void matrix_scalar_addition(Matrix* Input, float scalar, Matrix* Output){
    if (Input->rows != Output->rows || Input->cols != Output->cols) {
        FATAL_ERROR("Dimension mismatch in matrix_scalar_addition.");
    }
...
void matrix_hadamard(Matrix* a, Matrix* b, Matrix* Output) { 
    if (a->rows != b->rows || a->cols != b->cols || 
        a->rows != Output->rows || a->cols != Output->cols) {
        FATAL_ERROR("Matrices must be the same size for element-wise multiplication.");
    }
...
void matrix_normalize(Matrix* Dataset, Matrix* Output) {
    if (Dataset->rows != Output->rows || Dataset->cols != Output->cols) {
        FATAL_ERROR("Dimension mismatch in matrix_normalize.");
    }

    int total = Dataset->rows * Dataset->cols;
    if (total <= 0) {
        FATAL_ERROR("Cannot normalize an empty matrix.");
    }
    ...
    if (range == 0.0f) {
        FATAL_ERROR("Cannot normalize matrix with zero range (max == min). Division by zero.");
    }

    float inv_range = 1.0f / range;
    for (int i = 0; i < total; i++) {
        Output->data[i] = (Dataset->data[i] - min) * inv_range;
    }
}
...
void get_row(Matrix* Input, int row_index, Matrix* Output) {
    ...
    if (Output->rows != 1 || Output->cols != Input->cols) {
        FATAL_ERROR("Output matrix must be of shape (1, %d).", Input->cols);
    }

    int offset = row_index * Input->cols;
    for (int i = 0; i < Input->cols; i++) {
        Output->data[i] = Input->data[offset + i];
    }
}
