 ...
 *   Returns: An FFN_Layer with weights initialized using He initialization
 * Note: Must call free_ffn_layer() when done
 */
FFN_Layer init_ffn_layer(int d_model, int d_ff);

/**
 * free_ffn_layer - Release memory allocated for an FFN_Layer
 * 
 ...
 * Note: Must call free_ffn_cache() when done
 */
FFN_Cache init_ffn_cache(Matrix X, int d_model, int d_ff);
/**
 * free_ffn_cache - Release memory allocated for an FFN_Cache
 * 
...
/**
 * free_ffn_gradients - Release memory allocated for FFN_Gradients
 * 
