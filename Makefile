...
LIB_SOURCES = $(SRC_DIR)/matrix.c $(SRC_DIR)/gradient_descent.c $(SRC_DIR)/data_loader.c $(SRC_DIR)/vocabulary.c
ALL_HEADERS = $(INC_DIR)/matrix.h $(INC_DIR)/gradient_descent.h $(INC_DIR)/data_loader.h $(INC_DIR)/vocabulary.h
...
%.test: $(TEST_DIR)/%.c $(LIB_SOURCES) $(ALL_HEADERS)
	@echo -e "$(MAGENTA)Running test: $< $(RESET)"
	$(CC) $(CFLAGS) $< $(LIB_SOURCES) -o $* $(LDFLAGS)
	./$*
...
clean:
	@rm -f main xor_problem iris_dataset *.mem *.o *.memcheck test_vocab comprehensive_testing
